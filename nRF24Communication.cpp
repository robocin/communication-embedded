...
void nRF24Communication::_configure() {
  ...
  if (_config.function == RadioFunction::network0) // Robot Receiver
  {
  ...
  } else if (_config.function == RadioFunction::network1) {
    // Init communication as receiver
  ...
  // else if (_config.function == RadioFunction::network1) // Robot Sender
  // {
  //   this->_radio.openWritingPipe(_config.addr[0]);
  //   this->_radio.openReadingPipe(1, _config.addr[1]);
  //   this->_radio.setChannel(this->_config.sendChannel);
    ...
    while (this->_radio.available(&(_config.pipeNum))) {
      ...
      if (this->_rx.decoded.id == this->_robotId) {
        ...
        if (this->_typeMsg == msgType::VSS_SPEED) {
          ...
          this->_pos.maxSpeed =
              static_cast<double>((this->_mPostion.decoded.maxSpeed) / 1000.0);
          this->_pos.minSpeed =
              static_cast<double>((this->_mPostion.decoded.minSpeed) / 1000.0);
          this->_pos.rotateKp =
              static_cast<double>((this->_mPostion.decoded.rotateKp) / 100.0);
          this->_pos.usingPropSpeed =
              static_cast<bool>(this->_mPostion.decoded.usingPropSpeed);
          this->_pos.minDistanceToPropSpeed = static_cast<double>(
              (this->_mPostion.decoded.minDistanceToPropSpeed) / 1000.0);
          this->_pos.rotateInClockWise =
              static_cast<bool>(this->_mPostion.decoded.clockwise);
          this->_pos.orbitRadius = static_cast<double>(
              (this->_mPostion.decoded.orbitRadius) / 1000.0);
          this->_pos.approachKp =
              static_cast<double>((this->_mPostion.decoded.approachKp) / 100.0);
          ...
          this->_kick.kickStrength =
              static_cast<float>((this->_mPostion.decoded.strength) / 10.0);
          this->_kick.dribbler =
              static_cast<bool>(this->_mPostion.decoded.dribbler);
          this->_kick.dribblerSpeed = static_cast<float>(
              (this->_mPostion.decoded.dribblerSpeed) / 10.0);
        } else {
          break;
        }
        this->disable();
